...
float find_intersection(vec3 dir,vec3 pos,out vec3 normal)
{
	...
	intersect_first_plane(dir,pos,normalize(vec3(0.0,1.,0.0)),10.,near,far,normal);
	if(far<0.0 || near>far)
		return -1.0;
	
	const float faces = 4.0;
	...
	for(float i = 0.0;i < faces;i++)
	{
		...
		if(far<0.0 || near>far) return -1.0;
		//sds
		float aa = vec3(1.0,0.0,1.0)*m;
		intersect_plane(dir,pos,normalize(vec3(a, 0.0,a)),1.0,near,far,normal);
		if(far<0.0 || near>far) return -1.0;
	}
